...
if __name__ == '__main__':
    data_loader = AGNET_CTW(split='train',
        is_transform=False,
        img_size=640,
        short_size=640,
        kernel_scale=0.7,
    ...
    for _, data in enumerate(train_loader):
        gt_text = data['gt_texts']
        #gt_kernel = data['gt_kernels']
        #emb = data['gt_instances']
        gt_text = gt_text.data.cpu().numpy()[0].astype(np.uint8)
        #kernels = gt_kernel.data.cpu().numpy()[0].astype(np.uint8)
        #emb = emb.cpu().numpy()[0].astype(np.float32)
        #emb = (emb * 255).astype(np.uint8)
        #emb = cv2.cvtColor(emb, cv2.COLOR_GRAY2RGB)
        #heat_emb = cv2.applyColorMap(emb, cv2.COLORMAP_JET)  # 此处的三通道热力图是cv2使用GBR排列
        cv2.imwrite('gt.jpg', gt_text * 255)
       # cv2.imwrite('gt_kernel.jpg', kernels[0,:,:] * 255)
        #cv2.imwrite('emb.jpg', heat_emb)
