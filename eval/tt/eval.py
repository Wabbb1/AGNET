...
def one_to_many(local_sigma_table, local_tau_table, local_accumulative_recall,
                local_accumulative_precision, global_accumulative_recall, global_accumulative_precision,
                gt_flag, det_flag, num_gt,  tr, tp, fsc_k, k):
    for gt_id in range(num_gt):
...
def many_to_many(local_sigma_table, local_tau_table, local_accumulative_recall,
                 local_accumulative_precision, global_accumulative_recall, global_accumulative_precision,
                 gt_flag, det_flag, num_det, tr, tp, fsc_k, k):
...
def eval_batch(checkpoint_name):
    project_root = ''
            ...
            for gt_id, gt in enumerate(groundtruths):
                if len(detections) > 0:
                    for det_id, detection in enumerate(detections):
                        ...
                        # point1 = np.squeeze(gt[1])
                        # point2 = np.squeeze(gt[3])
                        # gt_p = np.concatenate((np.array(point1), np.array(point2)))
                        # gt_p = gt_p.reshape(2, -1).transpose()
                        # gt_p = plg.Polygon(gt_p)
    ...
    print('pb')
    with open("", 'a+') as f:
        f.write(str(checkpoint_name))
        f.write('p: %.4f, r: %.4f, f: %.4f' % (precision, recall, hmean))
        f.write('\n')
        f.close()
